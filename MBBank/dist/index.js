...
var MB = class {
  ...
  async login() {
    ...
    headers["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"; // Updated to Chrome 120
    ...
    console.log('DEBUG CAPTCHA RES (Local):', JSON.stringify(captchaRes));
    
    // Check if challenge
    if (captchaRes['sec-cp-challenge']) {
        throw new Error("MBBank Security Challenge functionality (Anti-Bot) triggered. Please retry or try Puppeteer.");
    }
    
    if (!captchaRes.imageString) {
         throw new Error("Missing Captcha Image. Response: " + JSON.stringify(captchaRes));
    }
